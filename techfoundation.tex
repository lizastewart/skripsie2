...
Specific thermal properties of timber need to be thoroughly understood to allow for accurate modelling interpretation of the results.
After 100 $^{^\circ}$C, the temperature exposure causes the breakdown of the timber structure in such a way that the structure will not return to the same state after cooling.
This breakdown is generally referred to as thermal degradation.
The degradation can be broken into four categories \citep{White:2001} \citep{Shi:2021}.
These four categories are based on the different reactions that take place and briefly summarised below.
...
Temperatures after 300 $^{^\circ}$C and before 450 $^{^\circ}$C are significant due to the amount of flamable volatiles released and the break of carbon linkages at 370 $^{^\circ}$C.
At temperatures higher than 450 $^{^\circ}$C the remaining timber is char and any further degradation is due to oxidation from carbon-monoxide, - dioxide and water.
...
All the thermal properties of various timber species at high temperatures were discussed in a research article by \citet{Shi:2021}.
 ...
 For this project the temperature of char will be used as 300 $^{^\circ}$C in accordance with the research of \citeauthor{Westhuyzen:2020}. 
% The specific heat ($C_p$) will be assumed to be constant at 1500 kJ/$\text{m}^3$/K. the density of pine was also assumed constant at 
 Using the equations in by \citeauthor{Shi:2021} the following $\kappa$-values (Equation \ref{shiK}) are obtained. 
