...
The code for the adapted model function, \texttt{model\_kinput.m} can be seen in Appendix \ref{codeapp}.
	...
	The defining of variables was moved into a new function that defines all the global variables used within the original \texttt{main.m} as well as the measured temperature data. 
...
By multiplying the $\mu$ vector with $((0.5+\vec{\text{rand}})\times 1.5)$ the first values of $x$ are guaranteed to be within an acceptable range of the prior values.
...
%	
	...
	The $i$ value was chosen to be 50 arbitrarily, initially $\delta = 0.5$ but as can be seen in Figure \ref{expcurvefig} this acceptance was still too lenient.
	\begin{equation}\label{acceptcriteq1}
	\exp\left(\frac {\pi^*(x_{n+1})-\pi^*(x_n)}{\pi^*(x_{\text{ref}})\delta}\right)\\
	\text{with} \quad \delta = 0.02\\
	\end{equation}
