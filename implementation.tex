...
\item{Thermal behaviour is isotropic throughout the model.}
	...
	The variables $c^A$ and $d^B$ are independent of $\vec{x}$ and can therefore be taken out of the integral. 
	...
	TODO: element --> model.
	This element is then overlaid with the timber element to effectively add an air element at each end of the timber element as depicted in Figure~\ref{airelmfig}.
...
TODO:  figure uit wat is d0.  The vectors $\vec{d}$ and $\vec{\dot{d}}$ can be obtained by setting $\vec{d}_0$ equal to the initial boundary conditions, setting $\vec{V}_0 = 0$, and integrating over time according to v-form time integration in Chapter 8 in \textit{The Finite Element Method} by \citeauthor{Hughes:1987} shown in detail in Appendix \ref{appder}.
...
By multiplying the $\mu$ vector with $1.5\times(0.5+\vec{\text{rand}})$, the first values of $x$ are guaranteed to be within an acceptable range of the prior values.
...
The prior function in this case was relatively easy to generate and incorporate into the program as a well-defined list of prior values exists.
	...
	\subsection{Likelihood probability function}
		...
		\pi(T|x) \propto \exp\left(-\frac{\left(T-\mathcal{M}(x)\right)^2}{2\sigma_{\text{temp}}^2}\right)
		\end{equation}
		...
		This function will output the probability of the modelled values $\mathcal{M}(x)$ given the measured temperature values ($T$).
		As can be seen in Equation~\ref{likelihoodfunc}, the $\mathcal{M}(x)$ vector is written as a function.
...
%	
	...
	The $i$ value was arbitrarily chosen to be 50, and initially $\delta = 0.5$ but as can be seen in Figure \ref{expcurvefig} this acceptance was still too lenient.
	\begin{equation}\label{acceptcriteq1}
	\exp\left(\frac {\pi^*(x_{n+1})-\pi^*(x_n)}{\pi^*(x_{\text{ref}})\delta}\right)\\
	\ \text{with} \ \delta = 0.02\\
	\end{equation}
	...
	The program was run four times with 5 000 iterations each time. 
...
All the values and constants except for the $\kappa$-values were kept the same as in the MCMC integration to allow the results to be compared on the same basis.  
	...
	The starting point for the algorithm was chosen to be the $\kappa$-values from the Eurocode.
